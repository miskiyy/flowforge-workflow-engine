...
export function Timeline({ events }: { events: RealtimeEvent[] }) {
  if (events.length === 0) {
    return (
      <p data-testid="timeline-empty" style={{ color: 'var(--ink-mut)' }}>
        No events yet.
      </p>
    );
  }
  ...
  return (
    <>
      {events.length > MAX_RENDERED_EVENTS ? (
        <p data-testid="timeline-capped-note" style={{ color: 'var(--ink-mut)', fontSize: 12 }}>
      ...
      <ol data-testid="timeline" style={{ listStyle: 'none', margin: 0, padding: 0 }}>
        {ordered.map((event) => (
          <li key={event.seq} data-testid="timeline-entry" style={{ padding: '4px 0', fontSize: 13 }}>
            <time dateTime={event.ts} style={{ color: 'var(--ink-mut)', marginRight: 8 }}>
